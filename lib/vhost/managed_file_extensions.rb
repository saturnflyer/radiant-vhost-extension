module Vhost::ManagedFileExtensions
 def full_filename(thumbnail = nil)
    ...
    filename = case self.thumbnail
      when "thumb"
        File.join(RAILS_ROOT, file_system_path, current_site.id.to_s, 'thumbs', thumbnail_name_for("thumb"))
      else
        File.join(RAILS_ROOT, file_system_path, current_site.id.to_s, thumbnail_name_for)
    end

    if thumbnail
      filename = File.join(RAILS_ROOT, file_system_path, current_site.id.to_s, 'thumbs', thumbnail_name_for("thumb"))
    end
